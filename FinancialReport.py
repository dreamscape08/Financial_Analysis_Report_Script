...
# output file is default to op-file-name=f"Report{presentday}"
#%%
import csv
from datetime import date
presentday=date.today()
op_file_name = f"Report: {presentday}"
csvpath = input('PATH_2_DATASET_#DEFAULT=budget_data.csv')
with open(csvpath, 'r') as revenue_data:
    ...
    print(output)
    with open(op_file_name,"w") as data:
        data.write(output)
